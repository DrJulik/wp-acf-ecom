<?php
/**
 * acf-test functions and definitions.
 *
...
if ( ! function_exists( '_setup' ) ) :
	...
	function _setup() {
		 ...
		 * If you're building a theme based on acf-test, use a find and replace
		 * to change 'acf-test' to the name of your theme in all the template files.
		 * You will also need to update the Gulpfile with the new text domain
		 * and matching destination POT file.
		 */
		load_theme_textdomain( 'acf-test', get_template_directory() . '/languages' );
		...
		register_nav_menus(
			array(
				'primary' => esc_html__( 'Primary Menu', 'acf-test' ),
				'mobile'  => esc_html__( 'Mobile Menu', 'acf-test' ),
			)
		);
		...
		add_theme_support( 'editor-color-palette', _get_theme_colors_gutenberg() );
		...
		add_theme_support(
			...
			array(
				array(
					'name' => __( 'Small', 'acf-test' ),
					'size' => 12,
					'slug' => 'small',
				),
				array(
					'name' => __( 'Normal', 'acf-test' ),
					'size' => 16,
					'slug' => 'normal',
				),
				array(
					'name' => __( 'Large', 'acf-test' ),
					'size' => 36,
					'slug' => 'large',
				),
				array(
					'name' => __( 'Huge', 'acf-test' ),
					'size' => 50,
					'slug' => 'huge',
				),
			)
...
function _widgets_init() {
	...
	// Define sidebars.
	$sidebars = array(
		'sidebar-1' => esc_html__( 'Sidebar 1', 'acf-test' ),
	);
	...
	foreach ( $sidebars as $sidebar_id => $sidebar_name ) {
		register_sidebar(
			array(
				'name'          => $sidebar_name,
				'id'            => $sidebar_id,
				'description'   => /* translators: the sidebar name */ sprintf( esc_html__( 'Widget area for %s', 'acf-test' ), $sidebar_name ),
