...
function _categorized_blog() {
	...
	$category_count = get_transient( '_categories' );
	...
	if ( false === $category_count ) {
		...
		set_transient( '_categories', $category_count );
	}
...
function _get_placeholder_unsplash( $args = array() ) {
	...
	if ( ! is_numeric( $args['width'] ) ) {
		throw new Exception( esc_html__( 'Width must be an integer', 'acf-test' ) );
	}

	if ( ! is_numeric( $args['height'] ) ) {
		throw new Exception( esc_html__( 'Height must be an integer', 'acf-test' ) );
	}
	...
	$alt = apply_filters( '_placeholder_image_alt', esc_html__( 'WebDevStudios Placeholder Image', 'acf-test' ) );
...
add_shortcode( '_copyright_year', '_copyright_year', 15 );
...
if ( defined( 'WPSEO_VERSION' ) ) {
	...
	add_filter( 'wpseo_metabox_prio', '_move_yoast_to_bottom' );
}
 ...
 * @author jomurgel
 */
function _get_the_content( $content ) {
...
add_filter( 'the_content', '_get_the_content', 20 );
 ...
 */
function _get_theme_colors_gutenberg() {
