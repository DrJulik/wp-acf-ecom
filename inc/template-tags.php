...
if ( ! function_exists( '_posted_on' ) ) :
	...
	function _posted_on() {
		...
		$posted_on = sprintf(
			...
			esc_html_x( 'Posted on %s', 'post date', 'acf-test' ),
			'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
		);

		$byline = sprintf(
			/* translators: the post author */
			esc_html_x( 'by %s', 'post author', 'acf-test' ),
...
if ( ! function_exists( '_entry_footer' ) ) :
	...
	function _entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			$categories_list = get_the_category_list( esc_html__( ', ', 'acf-test' ) );
			if ( $categories_list && _categorized_blog() ) {
				/* translators: the post category */
				printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'acf-test' ) . '</span>', $categories_list ); // WPCS: XSS OK.
			}

			/* translators: used between list items, there is a space after the comma */
			$tags_list = get_the_tag_list( '', esc_html__( ', ', 'acf-test' ) );
			if ( $tags_list ) {
				/* translators: the post tags */
				printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'acf-test' ) . '</span>', $tags_list ); // WPCS: XSS OK.
			}
		}

		if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
			echo '<span class="comments-link">';
			comments_popup_link( esc_html__( 'Leave a comment', 'acf-test' ), esc_html__( '1 Comment', 'acf-test' ), esc_html__( '% Comments', 'acf-test' ) );
			echo '</span>';
		}
 ...
 * @author WDS
 */
function _display_svg( $args = array() ) {
	echo _get_svg( $args ); // WPCS XSS Ok.
}
...
function _get_svg( $args = array() ) {
	...
	if ( empty( $args ) ) {
		return esc_html__( 'Please define default parameters in the form of an array.', 'acf-test' );
	}

	// Define an icon.
	if ( false === array_key_exists( 'icon', $args ) ) {
		return esc_html__( 'Please define an SVG icon filename.', 'acf-test' );
	}
	...
	<svg
	<?php
		echo _get_the_content( $height ); // WPCS XSS OK.
		echo _get_the_content( $width ); // WPCS XSS OK.
		echo _get_the_content( $fill ); // WPCS XSS OK.
		echo _get_the_content( $stroke ); // WPCS XSS OK.
		echo _get_the_content( $stroke_width ); // WPCS XSS OK.
	?>
		class="icon <?php echo esc_attr( $args['icon'] ); ?>"
	<?php
		echo _get_the_content( $aria_hidden ); // WPCS XSS OK.
		echo _get_the_content( $aria_labelledby ); // WPCS XSS OK.
	?>
		role="img">
 ...
 * @return bool
 */
function _display_copyright_text() {
	...
	echo _get_the_content( do_shortcode( $copyright_text ) ); // phpcs: xss ok.
}
 ...
 * @return string
 */
function _get_twitter_share_url() {
 ...
 * @return string
 */
function _get_facebook_share_url() {
 ...
 * @return string
 */
function _get_linkedin_share_url() {
	return add_query_arg(
		array(
 ...
 * @author WDS
 */
function _display_social_network_links() {
			...
			?>
				<li class="social-icon <?php echo esc_attr( $network ); ?>">
					<a href="<?php echo esc_url( $network_url ); ?>">
						...
						<?php
							echo /* translators: the social network name */ sprintf( esc_html( 'Link to %s', 'acf-test' ), ucwords( esc_html( $network ) ) ); // WPCS: XSS OK.
						?>
						</span>
					</a>
				</li><!-- .social-icon -->
 ...
 */
function _display_card( $args = array() ) {
	...
	<div class="<?php echo esc_attr( $args['class'] ); ?> card">
		...
		<a href="<?php echo esc_url( $args['url'] ); ?>" tabindex="-1">
			...
			<?php else : ?>
				<img src="<?php echo esc_url( get_stylesheet_directory_uri() ); ?>/dist/images/placeholder.png" class="card-image" loading="lazy" alt="<?php echo sprintf( esc_attr( 'Featured image for %s', 'acf-test' ), esc_attr( $args['title'] ) ); ?>">
			<?php endif; ?>
		</a>
		...
		<?php if ( $args['url'] ) : ?>
			<a class="button button-card" href="<?php echo esc_url( $args['url'] ); ?>"><?php esc_html_e( 'Read More', 'acf-test' ); ?></a>
		<?php endif; ?>
 ...
 * @return string
 */
function _display_header_button() {
	...
	$button_url  = get_theme_mod( '_header_button_url' );
	$button_text = get_theme_mod( '_header_button_text' );
	?>
	<div class="site-header-action">
		...
		?>
			<a href="<?php echo esc_url( $button_url ); ?>" class="button button-link"><?php echo esc_html( $button_text ?: __( 'More Information', 'acf-test' ) ); ?></a>
		<?php else : ?>
			<button type="button" class="cta-button" aria-expanded="false" aria-label="<?php esc_html_e( 'Search', 'acf-test' ); ?>">
				<?php esc_html_e( 'Search', 'acf-test' ); ?>
			</button>
 ...
 * @author Corey Collins
 */
function _display_numeric_pagination( $args = array() ) {
	...
	<nav class="pagination-container container" aria-label="<?php esc_html_e( 'numeric pagination', 'acf-test' ); ?>">
...
function _display_mobile_menu() {
	...
	<nav class="off-canvas-container" aria-label="<?php esc_html_e( 'Mobile Menu', 'acf-test' ); ?>" aria-hidden="true" tabindex="-1">
		<button type="button" class="off-canvas-close" aria-label="<?php esc_html_e( 'Close Menu', 'acf-test' ); ?>">
 ...
 * @return bool
 */
function _has_array_key( $key, $array = array() ) {
