...
function _acf_init() {
	...
	acf_register_block_type(
		array(
			'name'            => 'wds-accordion',
			'title'           => esc_html__( 'Accordion', 'acf-test' ),
			'description'     => esc_html__( 'A custom set of collapsable accordion items.', 'acf-test' ),
			'render_callback' => '_acf_block_registration_callback',
			'category'        => 'wds-blocks',
			'icon'            => 'sort',
			'keywords'        => array( 'accordion', 'wds' ),
			'mode'            => 'preview',
			'enqueue_assets'  => '_acf_enqueue_accordion_scripts',
			'align'           => 'wide',
			'supports'        => $supports,
			'example'         => array(
				'attributes' => array(
					'data' => array(
						'title'           => esc_html__( 'Accordion Block Title', 'acf-test' ),
						'text'            => esc_html__( 'Accordion Block Content', 'acf-test' ),
						'accordion_items' => array(
							'0' => array(
								'accordion_title' => esc_html__( 'Accordion Item Title', 'acf-test' ),
								'accordion_text'  => esc_html__( 'Accordion Item Content', 'acf-test' ),
							),
						),
					),
				),
	...
	acf_register_block_type(
		array(
			'name'            => 'wds-carousel',
			'title'           => esc_html__( 'Carousel', 'acf-test' ),
			'description'     => esc_html__( 'A carousel with a call to action for each slide.', 'acf-test' ),
			'render_callback' => '_acf_block_registration_callback',
			'category'        => 'wds-blocks',
			'icon'            => 'slides',
			'keywords'        => array( 'carousel', 'slider', 'wds' ),
			'mode'            => 'preview',
			'enqueue_assets'  => '_acf_enqueue_carousel_scripts',
			'align'           => 'wide',
			'supports'        => $supports,
			'example'         => array(
				'attributes' => array(
	...
	acf_register_block_type(
		array(
			'name'            => 'wds-cta',
			'title'           => esc_html__( 'Call To Action', 'acf-test' ),
			'description'     => esc_html__( 'A call to action block.', 'acf-test' ),
			'render_callback' => '_acf_block_registration_callback',
			'category'        => 'wds-blocks',
			'icon'            => 'megaphone',
			'keywords'        => array( 'call to action', 'cta', 'wds' ),
			'mode'            => 'preview',
			'enqueue_assets'  => '_acf_enqueue_backend_block_styles',
			'align'           => 'wide',
			'supports'        => $supports,
			'example'         => array(
				'attributes' => array(
					'data' => array(
						'title'       => esc_html__( 'Call To Action Title', 'acf-test' ),
						'text'        => esc_html__( 'Call To Action Text', 'acf-test' ),
						'button_link' => array(
							'title' => esc_html__( 'Learn More', 'acf-test' ),
							'url'   => '#',
						),
					),
				),
	...
	acf_register_block_type(
		array(
			...
			'title'           => esc_html__( 'Fifty/Fifty Block', 'acf-test' ),
			'description'     => esc_html__( 'A split-content block with text and/or media.', 'acf-test' ),
			'render_callback' => '_acf_block_registration_callback',
			'category'        => 'wds-blocks',
			'icon'            => 'editor-table',
			...
			'example'         => array(
				'attributes' => array(
					'data' => array(
						'text_primary' => esc_html__( 'Primary Text content', 'acf-test' ),
						'media_right'  => array(
	...
	acf_register_block_type(
		array(
			'name'            => 'wds-hero',
			'title'           => esc_html__( 'Hero Block', 'acf-test' ),
			'description'     => esc_html__( 'A hero with an optional call to action.', 'acf-test' ),
			'render_callback' => '_acf_block_registration_callback',
			'category'        => 'wds-blocks',
			'icon'            => 'slides',
	...
	acf_register_block_type(
		array(
			...
			'title'           => esc_html__( 'Recent Posts Block', 'acf-test' ),
			'description'     => esc_html__( 'A set of recent posts displayed by category and/or tag.', 'acf-test' ),
			'render_callback' => '_acf_block_registration_callback',
			'category'        => 'wds-blocks',
			'icon'            => 'admin-page',
	...
	acf_register_block_type(
		array(
			...
			'title'           => esc_html__( 'Related Posts Block', 'acf-test' ),
			'description'     => esc_html__( 'A set of manually selected posts.', 'acf-test' ),
			'render_callback' => '_acf_block_registration_callback',
			'category'        => 'wds-blocks',
			'icon'            => 'admin-page',
 ...
 * @author Corey Collins
 */
function _acf_block_registration_callback( $block ) {
	...
	if ( ! is_admin() && _has_block_expired(
		array(
	...
	_display_expired_block_message();
 ...
 * @author Corey Collins
 */
function _add_block_categories( $categories, $post ) {
	...
	return array_merge(
		...
		array(
			array(
				'slug'  => 'wds-blocks',
				'title' => esc_html__( 'WDS Blocks', 'acf-test' ),
			),
		)
	);
}
add_filter( 'block_categories', '_add_block_categories', 10, 2 );
 ...
 * @author Corey Collins
 */
function _acf_enqueue_backend_block_styles() {
 ...
 * @author Corey Collins
 */
function _acf_enqueue_carousel_scripts() {

	if ( ! is_admin() ) {
		return;
	}

	_acf_enqueue_backend_block_styles();
 ...
 * @author Corey Collins
 */
function _acf_enqueue_accordion_scripts() {
 ...
 * @author Corey Collins
 */
function _get_block_alignment( $block ) {
 ...
 * @author Corey Collins
 */
function _get_block_classes( $block ) {
	...
	$classes  = '';
	$classes  = _get_block_expired_class();
 ...
 * @author Corey Collins
 */
function _get_block_expired_class() {
 ...
 * @author Corey Collins
 */
function _display_expired_block_message() {

	if ( ! _get_block_expired_class() ) {
		return;
	}

	?>
	<div class="block-expired-message">
		<span class="block-expired-text"><?php esc_html_e( 'Your block has expired. Please change or remove the Start and End dates under Other Options to display your block on the frontend.', 'acf-test' ); ?></span>
	</div>
	<?php
}
 ...
 * @author Corey Collins
 */
function _acf_gutenberg_display_admin_default_carousel( $block ) {
	...
	for ( $slides = 0; $slides < 2; $slides++ ) :
		...
		<section class="slide">
			<div class="slide-content container">
				<h2 class="slide-title"><?php esc_html_e( 'Slide Title', 'acf-test' ); ?></h2>
				<p class="slide-description"><?php esc_html_e( 'Slide Content', 'acf-test' ); ?></p>
			</div>
		</section>
	<?php
	endfor;
