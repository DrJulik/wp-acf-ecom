 ...
 */
function _display_block_options( $args = array() ) {
	...
	// Get the block ID.
	$block_id = _get_block_id( $args['block'] );
	...
	$has_show_overlay = _has_array_key( 'show_overlay', $background_options ) && true === $background_options['show_overlay'] ? ' has-overlay' : '';
	...
	if ( $args['background_type'] ) {
		...
		if ( 'video' === $args['background_type'] ) {
			...
			$background_alt = $background_title ? sprintf( esc_attr( 'Video Background of %s', 'acf-test' ), esc_attr( $background_options['background_video_title'] ) ) : __( 'Video Background', 'acf-test' );
			...
			?>
				...
				<button class="video-toggle"><span class="screen-reader-text"><?php esc_html_e( 'Toggle video playback', 'acf-test' ); ?></span></button>
			<?php
 ...
 * @return bool
 */
function _has_block_expired( $args = array() ) {
 ...
 */
function _acf_flexible_content_layout_title( $block_title, $field, $layout, $i ) {
	...
	if ( 'none' !== $background ) {
		...
		$type = _return_flexible_content_layout_value( $background_type );
		...
		if ( 'video' === $background_type ) {
			$block_heading .= '<div style="font-size: 30px; height: 26px; width: 30px;" class="dashicons dashicons-format-video acf-flexible-title-image"><span class="screen-reader-text">' . esc_html__( 'Video', 'acf-test' ) . '</span></div>';
		}
	}
	...
	if ( _has_block_expired(
		...
		$expired .= '<span style="color: red;">&nbsp;(' . esc_html__( 'expired', 'acf-test' ) . ')</span>';
	}
...
add_filter( 'acf/fields/flexible_content/layout_title/name=content_blocks', '_acf_flexible_content_layout_title', 10, 4 );
 ...
 */
function _return_flexible_content_layout_value( $type ) {
...
if ( function_exists( '_acf_flexible_content_layout_title' ) ) {

	/**
	 * Set Admin Styles for Flexible Content Layout Image/Title in _acf_flexible_content_layout_title().
	 *
	 * @author WDS
	 */
	function _flexible_content_layout_title_acf_admin_head() {
	?>
	<style type="text/css">
	...
	add_action( 'acf/input/admin_head', '_flexible_content_layout_title_acf_admin_head' );
}

/**
 * Load colors dynamically into select field from _get_theme_colors()
 *
 * @author WDS
 ...
 */
function _acf_load_color_picker_field_choices( $field ) {
...
add_filter( 'acf/load_field/name=color_picker', '_acf_load_color_picker_field_choices' );
 ...
 */
function _get_theme_colors() {
	return array(
		esc_html__( 'Alto', 'acf-test' )           => '#ddd',
		esc_html__( 'Black', 'acf-test' )          => '#000',
		esc_html__( 'Blue', 'acf-test' )           => '#21759b',
		esc_html__( 'Cod Gray', 'acf-test' )       => '#111',
		esc_html__( 'Dove Gray', 'acf-test' )      => '#666',
		esc_html__( 'Gallery', 'acf-test' )        => '#eee',
		esc_html__( 'Gray', 'acf-test' )           => '#808080',
		esc_html__( 'Gray Alt', 'acf-test' )       => '#929292',
		esc_html__( 'Light Yellow', 'acf-test' )   => '#fff9c0',
		esc_html__( 'Mineshaft', 'acf-test' )      => '#333',
		esc_html__( 'Silver', 'acf-test' )         => '#ccc',
		esc_html__( 'Silver Chalice', 'acf-test' ) => '#aaa',
		esc_html__( 'White', 'acf-test' )          => '#fff',
		esc_html__( 'Whitesmoke', 'acf-test' )     => '#f1f1f1',
	);
}
 ...
 * @return void
 */
function _display_hero_heading( $block_title ) {
 ...
 * @since NEXT
 */
function _display_link( $args = array() ) {
	echo _get_link( $args ); // WPCS: XSS Ok.
}
...
function _get_link( $args = array() ) {
	...
	$title = _has_array_key( 'title', $button_array ) ? $button_array['title'] : esc_html__( 'Read More', 'acf-test' );

	// Get url.
	$url = _has_array_key( 'url', $button_array ) ? $button_array['url'] : '';

	// Get target, else default internal.
	$target = _has_array_key( 'target', $button_array ) ? $button_array['target'] : '_self';
	?>
