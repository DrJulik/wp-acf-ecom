...
function _content_more_link() {
	return ' <a class="more-link" href="' . get_permalink() . '">' . esc_html__( 'Read More', 'acf-test' ) . '...</a>';
}
...
function _excerpt_more( $more ) {
	return sprintf( ' <a class="more-link" href="%1$s">%2$s</a>', get_permalink( get_the_ID() ), esc_html__( 'Read more...', 'acf-test' ) );
}
...
add_shortcode( 'svg', '_display_svg' );
 ...
 * @return string
 */
function _display_customizer_header_scripts() {

	// Check for header scripts.
	$scripts = get_theme_mod( '_header_scripts' );
	...
	echo _get_the_content( $scripts ); // WPCS XSS OK.
}
add_action( 'wp_head', '_display_customizer_header_scripts', 999 );
 ...
 * @return string
 */
function _display_customizer_footer_scripts() {

	// Check for footer scripts.
	$scripts = get_theme_mod( '_footer_scripts' );
...
add_action( 'wp_footer', '_display_customizer_footer_scripts', 999 );
...
function _add_og_tags() {
	...
	$default_base_description = ( get_bloginfo( 'description' ) ) ? get_bloginfo( 'description' ) : esc_html__( 'Visit our website to learn more.', 'acf-test' );
	...
	if ( is_category() || is_tag() || is_tax() ) {
		...
		$specify        = ( is_category() ) ? esc_html__( 'categorized in', 'acf-test' ) : esc_html__( 'tagged with', 'acf-test' );
		$queried_object = get_queried_object();
		$card_url       = get_term_link( $queried_object );
		$card_type      = 'website';

		// Translators: get the term name.
		$card_long_description = $card_description = sprintf( esc_html__( 'Posts %1$s %2$s.', 'acf-test' ), $specify, $term_name );
	}
	...
	if ( is_search() ) {
		...
		$card_long_description = $card_description = sprintf( esc_html__( 'Search results for %s.', 'acf-test' ), $search_term );
	}
...
add_action( 'wp_head', '_add_og_tags' );
 ...
 * @author Corey Collins
 */
function _remove_archive_title_prefix( $block_title ) {
...
add_filter( 'get_the_archive_title', '_remove_archive_title_prefix' );
 ...
 * @author Corey Collins
 */
function _remove_empty_p_tags_from_content( $content ) {
...
add_filter( 'the_content', '_remove_empty_p_tags_from_content' );
 ...
 * @author Corey Collins
 */
function _add_chrome_lazy_loading_to_images( $attr ) {

	$attr['loading'] = 'lazy';

	return $attr;
}
add_filter( 'wp_get_attachment_image_attributes', '_add_chrome_lazy_loading_to_images' );
 ...
 * @author Corey Collins
 */
function _allow_img_attributes( $allowedposttags ) {
...
add_filter( 'wp_kses_allowed_html', '_allow_img_attributes', 1 );
 ...
 * @author Corey Collins
 */
function _add_lazy_loading_to_image_block( $block_content, $block ) {
...
add_filter( 'render_block', '_add_lazy_loading_to_image_block', 10, 2 );
