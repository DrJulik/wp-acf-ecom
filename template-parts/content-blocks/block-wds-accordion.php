...
?>
	<div class="container">
		...
		<?php if ( $text ) : ?>
			<?php echo _get_the_content( $text ); // phpcs: xss: ok. ?>
		<?php endif; ?>

		<?php if ( $accordion_items ) : ?>
			<?php $count = 0; ?>
			<div class="accordion" aria-label="<?php esc_attr_e( 'Accordion Content Block', 'acf-test' ); ?>">
				<?php foreach ( $accordion_items as $accordion_item ) : ?>
					...
					<div class="accordion-item">
						<div class="accordion-item-header">
							<h3 class="accordion-item-title"><?php echo esc_html( $accordion_item['accordion_title'] ); ?>
								<button class="accordion-item-toggle" aria-expanded="false" aria-controls="<?php echo esc_attr( $item_content_id ); ?>">
									<span class="screen-reader-text"><?php echo sprintf( esc_html( 'Toggle %s', 'acf-test' ), esc_html( $accordion_item['accordion_title'] ) ); ?></span>
						...
						<div id="<?php echo esc_attr( $item_content_id ); ?>" class="accordion-item-content" aria-hidden="true">
							<?php echo _get_the_content( $accordion_item['accordion_text'] ); // phpcs: xss: ok. ?>
						</div>
					</div>
				<?php endforeach; ?>
			</div>
